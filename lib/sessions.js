...
function stateTransition(txnState, nextState) {
...
class ClientSession extends EventEmitter {
  ...
  endSession(options, callback) {
    ...
    if (this.serverSession && this.transaction.isActive) {
   ...
   * @param {ReadPreference} [options.readPreference] T
   */
  startTransaction(options) {
    ...
    this.transaction.options = Object.assign({}, options || this.defaultTransactionOptions);

    stateTransition(this.transaction, STARTING_TRANSACTION);
  }
...
function endTransaction(clientSession, commandName, callback) {
  ...
  let txnState = clientSession.transaction.state;
  if (txnState === NO_TRANSACTION) {
    callback(new MongoError('No transaction started'));
    return;
  }

  if (commandName === 'commitTransaction') {
    if (txnState === TRANSACTION_ABORTED) {
    ...
    if (txnState === TRANSACTION_COMMITTED) {
  ...
  function commandHandler(e, r) {
    if (commandName === 'commitTransaction') {
      stateTransition(clientSession.transaction, TRANSACTION_COMMITTED);
    } else {
      stateTransition(clientSession.transaction, TRANSACTION_ABORTED);
    }
