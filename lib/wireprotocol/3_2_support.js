...
function decorateWithTransactionsData(command, session, isRetryableWrite) {
  ...
  const transactionActive = session.transaction.isActive;

  if (serverSession.txnNumber && (isRetryableWrite || transactionActive)) {
...
WireProtocol.prototype.getMore = function(
  ...
  if (options.session && options.session.transaction.isActive) {
